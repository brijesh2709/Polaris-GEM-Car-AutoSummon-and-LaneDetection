...
from rrt_with_yaw import RRTWithYaw  # Assume a simple RRT module is implemented
...
class PurePursuit(object):
    ...
    def auto_summon_callback(self, msg):
        ...
        print("top left:", self.wps_to_local_xy(-88.235468, 40.092836))
        print("top right:", self.wps_to_local_xy(-88.235468, 40.092746))
        print("bottom right:", self.wps_to_local_xy(-88.236171, 40.092746))
        print("bottom left:", self.wps_to_local_xy(-88.236171, 40.092836))

        top_left = self.wps_to_local_xy(-88.235468, 40.092836)
        top_right = self.wps_to_local_xy(-88.235468, 40.092746)
        bottom_right =  self.wps_to_local_xy(-88.236171, 40.092746)
        bottom_left = self.wps_to_local_xy(-88.236171, 40.092836)

        print("top left:", utils.transform_to_image_coordinates(top_left[0], top_left[1], resolution=0.2))
        print("top right:", utils.transform_to_image_coordinates(top_right[0], top_right[1], resolution=0.2))
        print("bottom right:", utils.transform_to_image_coordinates(bottom_right[0], bottom_right[1], resolution=0.2))
        print("bottom left:", utils.transform_to_image_coordinates(bottom_left[0], bottom_left[1], resolution=0.2))
        ...
        print("GPS Coords")
        print(local_x_gps, local_y_gps )
        print("dest", destLocationImageCoordinates)
        print("Car Coords")
        print(local_curr_x, local_curr_y )
        print("curr", currLocationImageCoordinates)
        while self.rrt_path_found == False:
            ...
            if path!=None:
                ...
                path_points = self.transform_points_img_to_sim(points=path, resolution=0.2)
                ...
                # Write the generated waypoints to a CSV file
    ...
    def find_target_waypoint(self, curr_x, curr_y, curr_yaw):        
        ...
        if dist_to_target < 4: #and yaw_difference < 0.1:  # 2 meters and ~5.7 degrees
